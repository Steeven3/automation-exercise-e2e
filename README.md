...
1. **Créer une feature** dans `features/` au format Gherkin
```gherkin
Feature: Description
  Scenario: Nom du scénario
    Given étape initiale
    When action
    Then vérification
```

2. **Implémenter les steps** dans `steps/` correspondant à la feature

3. **Ajouter les méthodes** dans les page objects si nécessaire

4. **Exécuter** : `npm run test`

## ✅ Critères de qualité

- ✅ Projet fonctionnel et exécutable localement
- ✅ Scénarios Gherkin cohérents et lisibles
- ✅ Respect du modèle POM
...
1. Consulter les logs de sortie de la commande `npm run test`
2. Vérifier le rapport HTML généré : `reports/cucumber-report.html`
3. Consulter les captures d'écran attachées en cas d'erreur
2. Créer les steps correspondants dans `steps/`
3. Créer la page object si nécessaire dans `pages/`

## 📞 Support

Pour toute question ou problème, vérifier les logs des tests et les rapports HTML générés.
